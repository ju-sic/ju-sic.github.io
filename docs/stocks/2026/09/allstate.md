...
| **최종 투자의견** | **분할 매수 (Buy on Dip)** / 200일선($222) 지지 시험 구간에서 선발대 투입 (투자 가치 점수 **85.0점**, 6.1절) |
...
| **종합 가중치 환산 점수** | **100%** | — | **85.0점** | **`Buy`** | **펀더멘털과 밸류에이션은 최상급, 200일선 지지 확인하며 분할 매수** |
...
#### ① 지금 당장 사야 하는가? ➔ **"선발대만 던져라. 주력은 200일선 지지 양봉을 확인하고 실어라."**

- **선발대를 보내도 되는 이유**: 고점 대비 -18.6% 급락하며 200일선($222) 코앞까지 내려왔다. 감익을 반영한 선행 PER 8.1배는 피어 대비 30% 이상 싼 가격표다. 8월 재해 손실은 이미 공시되었고, 메타 뮤즈 공포는 아직 가입자 이탈 데이터로 확인되지 않았다.
- **주력을 바로 싣지 않는 이유**: 9월 18일부터 거래량을 동반한 하락이 이어졌고, 9월 23일은 저가에 마감했다. 단기 매도 관성이 며칠 더 이어질 수 있다. 떨어지는 칼날을 맨손으로 잡지 말고, 200일선($222) 부근에서 하방 경직성을 확인하면서 4회에 걸쳐 쪼개 사야 한다.
...
| :--- | :--- | :---: | :---: | :--- |
| **1차 진입 (선발대)** | 현 가격대, 200일선 근접 및 선행 PER 8배 초반 구간 | **25%** | **$222 ~ $228** | 거래량 동반 하락이 진행 중이므로 소규모 정찰대만 배치 |
| **2차 매수 (보강)** | AI 공포성 오버슈팅으로 200일선 일시 하회 시 | **25%** | **$210 ~ $218** | 투매 패닉 구간에서 평단가를 낮추는 용도 |
| **3차 매수 (주력)** | 200일선 지지 확인 후 거래량 감소 및 반등 양봉 출현 | **30%** | **$230 ~ $238** | 바닥 다지기 및 추세 복원 확인 후 핵심 주력 물량 투입 |
| **4차 매수 (추세 불타기)** | 50일 이동평균선($257) 회복 및 안착 시 | **20%** | **$258 돌파 시** | 2단계 상승 랠리 복귀 확인 시 마지막 잔여 물량 투입 |
| **손절 및 위험 관리선** | 장기 추세 완전 붕괴 또는 분기 합산비율 105% 초과 악화 | — | **$200 종가 하회** | 펀더멘털 훼손 시 미련 없이 손절 및 비중 축소 |

만약 2차 보강 가격대($210~$218)를 주지 않고 200일선($222) 위에서 곧바로 반등할 경우, 2차 배정 물량은 3차 주력 매수 구간($230~$238)에 합산하여 집행한다.
...
- **② 200일 이동평균선($222.09) 종가 지지 여부**: 주가가 200일선을 지켜내고 거래량이 줄며 반등하면 눌림목 성공으로 보고 3차 주력을 싣는다. 종가 기준으로 200일선을 깨면 3차 주력은 보류하고 $210~$218 구간의 2차 보강만 집행한다. $200 종가 이탈 시에는 손절한다.
...
!!! tip "최종 핵심 제언 (Executive Takeaway)"
    - **최종 투자 가치 점수**: **85.0점 / 100점** (`Buy` / 감익을 반영하고도 남는 저평가, 200일선 지지 시험 구간)
    ...
    - **행동 원칙**: 테크주 폭등을 쫓아다니며 뇌동매매하지 마라. 선발대는 지금, 주력은 200일선 지지 양봉 뒤에, 손절은 $200 종가 이탈에. 대중이 엉뚱한 공포에 질려 현금 요새를 던질 때 규칙대로 주워 담는 자만이 복리의 단열매를 누린다.
